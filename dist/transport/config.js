...
export function isHostedURL(value) {
    try {
...
export function resolveConfig(requestedModel, options, callMaxOutputTokens) {
    const apiKey = options.apiKey?.trim() || env("ADROUTER_API_KEY");
    if (!apiKey) {
        throw new Error("AdRouter authentication is not configured. Set ADROUTER_API_KEY or run `opencode auth login --provider adrouter`.");
    }
    ...
    return {
        ...
        workspace: env("ADROUTER_WORKSPACE") ??
            options.workspace ??
            (typeof process === "undefined" ? "." : process.cwd()),
        adMode: forcedOff ? "off" : (environmentAdMode ?? options.adMode ?? (isHostedURL(baseURL) ? "live" : "mock")),
        runtimeMode: isHostedURL(baseURL) ? undefined : configuredRuntime,
        adsEnabled,
